...
interface PasswordFieldBuilderProps {
  field: FormField;
  index: number;
  updateField: (index: number, field: FormField) => void;
}

const PasswordFieldBuilder: React.FC<PasswordFieldBuilderProps> = ({ field, index, updateField }) => {
  return (
...
<option value="radio">Radio</option>
        ...
        <div className="col-span-2">
          <label className="block text-sm font-medium text-theme-text">Pattern</label>
          <input
            type="text"
            value={field.pattern || ''}
            onChange={e => updateField(index, { ...field, pattern: e.target.value })}
            className="mt-1 block w-full rounded-md border-theme-border bg-theme-input text-theme-text"
          />
        </div>
...
export default PasswordFieldBuilder;
